...
void Application::CheckNewVersion(Ota &ota)
{
    ...
    int retry_count = 0;
    int retry_delay = 10; // Initial retry delay 10 seconds
    ...
    while (true)
    {
        ...
        if (!ota.CheckVersion())
        {
            ...
            retry_delay *= 2; // Double delay time after each retry
            continue;
        }
        retry_count = 0;
...
void Application::SetDeviceState(DeviceState state)
{
    ...
    device_state_ = state;
    ESP_LOGI(TAG, "TRẠNG THÁI: %s", STATE_STRINGS[device_state_]);
	...
	// Tạm dừng lại không phát radio nữa
	if (previous_state == kDeviceStateIdle && state != kDeviceStateIdle) {
    ...
    // QUAN TRỌNG: Khi ở trạng thái idle, đảm bảo tốc độ mẫu là 16000 Hz cho giọng nói
    if (codec && codec->output_sample_rate() != 16000) {
        ESP_LOGI(TAG, "Đặt tốc độ mẫu 16000 Hz cho trạng thái idle");
        codec->SetOutputSampleRate(16000);
    }
    break;
    ...
    case kDeviceStateListening:
        ...
        // Đảm bảo tốc độ mẫu là 16000 Hz cho giọng nói
        if (codec && codec->output_sample_rate() != 16000) {
            ESP_LOGI(TAG, "Đặt tốc độ mẫu 16000 Hz cho chế độ nghe");
            codec->SetOutputSampleRate(16000);
        }
    ...
    case kDeviceStateSpeaking:
        ...
        {
            audio_service_.EnableVoiceProcessing(false);
            // Chỉ từ đánh thức AFE có thể được phát hiện trong chế độ nói
...
// Add external audio data (like music playback)
void Application::AddAudioData(AudioStreamPacket &&packet)
{
    ...
    if (device_state_ == kDeviceStateIdle && codec->output_enabled())
    {
        ...
        if (packet.payload.size() >= 2)
        {
            ...
            // Check if sample rate matches, if not do simple resampling
            if (packet.sample_rate != codec->output_sample_rate())
            {
                ...
                if (packet.sample_rate > codec->output_sample_rate())
                {
                    ...
                    {
                        ESP_LOGW(TAG, "Unable to switch sample rate, continuing with current: %d Hz", codec->output_sample_rate());
                    }
                }
                else
                {
                    ...
                    for (size_t i = 0; i < pcm_data.size(); ++i)
                    {
                        ...
                        // Calculate number of interpolation samples needed
